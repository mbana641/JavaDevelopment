...
public class BinarySearchTree<T extends Comparable<T>> implements BSTInterface<T> {
	...
	// Calls recursive method countSingle to count the number
	// of single parents
	{
		return countSingle(root);
	}

	private int countSingle(BSTNode<T> tree) {
		// To-do
		if (tree == null)
			return 0;
		if (tree.right == null && tree.left != null || tree.left == null && tree.right != null)
			return countSingle(tree.left) + countSingle(tree.right) + 1;
		else
			return countSingle(tree.left) + countSingle(tree.right);
